...
export const GetRoasteriesQuerySchema = buildPaginationSchema({
  ...
  q: z.string().trim().min(1, "q must not be empty").max(64, "q is too long").optional(),
  city: z.string().trim().min(1, "city must not be empty").max(64, "city is too long").optional(),
});
