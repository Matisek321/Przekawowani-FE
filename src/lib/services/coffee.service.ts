...
/**
 * Parameters for listing coffees globally with optional filters.
 */
type ListCoffeesParams = {
	page: number
	pageSize: number
	roasteryId?: string
	qNorm?: string // normalized search query
}

/**
 * Lists coffees globally with ranking order (by avg rating desc).
 * Supports filtering by roastery and search by normalized name.
 *
 * @param supabase - The Supabase client instance
 * @param params - Filtering and pagination parameters
 * @returns Object containing items (CoffeeDto[]) and total count
 */
export async function listCoffees(
	supabase: SupabaseClient,
	params: ListCoffeesParams
): Promise<{ items: CoffeeDto[]; total: number }> {
	const { page, pageSize, roasteryId, qNorm } = params
	...
	let query = supabase
		...
		// Ranking sort: avg_main desc (nulls last), ratings_count desc, id desc for stability
	...
	if (roasteryId) {
		query = query.eq('roastery_id', roasteryId)
	}

	if (qNorm) {
		query = query.ilike('normalized_name', `%${qNorm}%`)
	}

	const { data, count, error } = await query

	if (error) {
		console.error('[coffee.service] Error listing coffees', { params, error })
		throw new CoffeeServiceError('server_error', 'Failed to list coffees')
	}

	// Map database rows to CoffeeDto, calculating smallSample
