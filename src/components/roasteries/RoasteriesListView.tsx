...
export interface RoasteriesQueryState {
  page: number;
  pageSize: number;
}

export interface RoasteryListItemVM {
  id: string;
  name: string;
  city: string;
  href: string;
...
export interface RoasteriesListVM {
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-4 py-12 text-center">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground">Nie znaleziono żadnych palarni. Dodaj pierwszą!</p>
      </div>
    </div>
  );
}
...
interface UseRoasteriesListResult {
...
interface RoasteriesListViewProps {
...
function parseQueryFromUrl(): RoasteriesQueryState {
  ...
  const pageSize =
    Number.isInteger(parsedPageSize) && parsedPageSize >= 1 && parsedPageSize <= 100 ? parsedPageSize : 20;
