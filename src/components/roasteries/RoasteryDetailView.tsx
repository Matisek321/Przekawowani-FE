...
export interface RoasteryCoffeesQueryState {
  page: number;
  pageSize: number;
}

export interface RoasteryDetailVM {
  id: string;
  name: string;
  city: string;
}

export interface CoffeesListVM {
...
interface UseRoasteryDetailResult {
...
interface UseRoasteryCoffeesResult {
...
function useRoasteryCoffees(roasteryId: string, query: RoasteryCoffeesQueryState): UseRoasteryCoffeesResult {
...
interface RoasteryDetailViewProps {
  roasteryId: string;
...
function parseQueryFromUrl(): RoasteryCoffeesQueryState {
  ...
  const pageSize =
    Number.isInteger(parsedPageSize) && parsedPageSize >= 1 && parsedPageSize <= 100 ? parsedPageSize : 30;
...
export function RoasteryDetailView({ roasteryId, initialQuery }: RoasteryDetailViewProps) {
  ...
  const updateUrl = useCallback(
    (newQuery: RoasteryCoffeesQueryState) => {
      const params = new URLSearchParams({
  ...
  if (roasteryDetail.error?.code === "roastery_not_found") {
    return (
      <div className="space-y-6">
        <ErrorBanner message={roasteryDetail.error.message} showBackLink />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {roasteryDetail.data && (
        <RoasteryHeader roastery={roasteryDetail.data} showAddCoffeeButton={showAddCoffeeButton} />
      )}

      {/* Error for coffees list */}
      {coffeesList.error && <ErrorBanner message={coffeesList.error.message} onRetry={handleRetry} />}
      ...
      {roasteryDetail.data &&
        !coffeesList.isLoading &&
        !coffeesList.error &&
        coffeesList.data &&
        coffeesList.data.items.length === 0 && (
