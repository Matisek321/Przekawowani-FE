...
vi.mock("@/components/roasteries/coffee/CreateCoffeeForm", () => ({
  CreateCoffeeForm: vi.fn(({ roasteryId, onSuccess }) => (
    <div data-testid="create-coffee-form" data-roastery-id={roasteryId}>
      Mocked Form
      <button onClick={() => onSuccess({ id: "coffee-123", name: "Test Coffee" })}>Trigger Success</button>
    </div>
  )),
}));
...
describe("CreateCoffeePage", () => {
  ...
  describe("Error States", () => {
    ...
    it("shows generic error when roastery fetch fails", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText("Wystąpił błąd podczas pobierania danych palarni")).toBeInTheDocument();
      });
    });
  ...
  describe("Success State - Form Rendering", () => {
    ...
    it("renders page title and description", async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText("Wprowadź nazwę nowej kawy dla wybranej palarni.")).toBeInTheDocument();
      });
    });
    ...
    it("renders MVP info banner", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/W wersji MVP nie można edytować ani usuwać kawy/i)).toBeInTheDocument();
      });
    });
