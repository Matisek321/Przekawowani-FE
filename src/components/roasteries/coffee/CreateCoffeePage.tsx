...
// ViewModel types
interface RoasteryContextVM {
  id: string;
  name: string;
  city: string;
}

interface UseRoasteryContextResult {
...
// Component props
interface CreateCoffeePageProps {
  roasteryId: string;
}
...
interface RoasteryContextProps {
    ...
    <div className="container mx-auto max-w-2xl px-4 py-8">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Wprowadź nazwę nowej kawy dla wybranej palarni.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <Alert>
            ...
            <AlertDescription>
              W wersji MVP nie można edytować ani usuwać kawy po jej utworzeniu. Upewnij się, że wprowadzone dane są
              poprawne.
            </AlertDescription>
          </Alert>
          ...
          {accessToken && (
            <CreateCoffeeForm roasteryId={roasteryId} accessToken={accessToken} onSuccess={handleSuccess} />
          )}
        </CardContent>
      </Card>
    </div>
