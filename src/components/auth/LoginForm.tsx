...
export function LoginForm() {
  ...
  const handleChange = useCallback(
    (field: keyof LoginFormState) => (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (touched[field]) {
        const fieldError = field === "email" ? validateEmail(newValue) : validatePassword(newValue);
  ...
  const handleBlur = useCallback(
    (field: keyof LoginFormState) => () => {
      ...
      const fieldError = field === "email" ? validateEmail(values[field]) : validatePassword(values[field]);
