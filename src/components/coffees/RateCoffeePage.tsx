...
// ViewModel types
interface CoffeeContextVM {
  id: string;
  name: string;
}

interface UseCoffeeContextResult {
...
interface CoffeeInfoHeaderProps {
  coffeeName: string;
  coffeeId: string;
}
    ...
    <div className="space-y-4">
      ...
      {showCoffeeLink && coffeeId && (
        <p className="text-center text-sm">
          <a href={`/coffees/${coffeeId}`} className="text-primary underline underline-offset-4 hover:text-primary/80">
      ...
      {!coffeeId && (
        <p className="text-center text-sm">
          <a href="/coffees" className="text-primary underline underline-offset-4 hover:text-primary/80">
            Wróć do listy kaw
          </a>
        </p>
      )}
...
function CoffeeInfoHeader({ coffeeName, coffeeId }: CoffeeInfoHeaderProps) {
  return (
    <div className="rounded-md bg-muted p-4">
      ...
      <a href={`/coffees/${coffeeId}`} className="font-medium text-primary hover:underline underline-offset-4">
        {coffeeName}
      </a>
    </div>
  );
...
export function RateCoffeePage({ coffeeId }: RateCoffeePageProps) {
  const { accessToken, isAllowed, isChecking, isRedirecting, isBlocked, gate } = useDisplayNameGate({
  ...
  return (
    <div className="container mx-auto max-w-2xl px-4 py-8">
      ...
      <Card className="mt-6">
        <CardHeader>
          <CardTitle className="text-2xl">{isEditing ? "Edytuj ocenę" : "Oceń kawę"}</CardTitle>
          <CardDescription>
            {isEditing
