...
// ViewModel types
interface SetDisplayNameFormErrors {
  displayName?: string;
  form?: string;
}

interface SetDisplayNameFormProps {
  accessToken: string;
...
export function SetDisplayNameForm({ accessToken, onSuccess }: SetDisplayNameFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6" noValidate data-test-id="set-display-name-form">
      ...
      {/* Submit button */}
      <Button type="submit" disabled={isSubmitting} className="w-full" data-test-id="set-display-name-submit-button">
        {isSubmitting ? (
          <>
